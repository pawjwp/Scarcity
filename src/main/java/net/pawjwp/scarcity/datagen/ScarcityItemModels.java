...
public class ScarcityItemModels extends ItemModelProvider {
    public static final String GENERATED = "item/generated";

    public ScarcityItemModels(PackOutput output, ExistingFileHelper existingFileHelper) {
        super(output, Scarcity.MOD_ID, existingFileHelper);
    }
    ...
    public ResourceLocation resourceItem(String path) {
        return ResourceLocation.fromNamespaceAndPath(Scarcity.MOD_ID, "item/" + path);
    }
    ...
    public static <T> Collection<T> takeAll(Set<? extends T> src, T... items) {
        ...
        for(T item : items) {
            if (!src.contains(item)) {
                Scarcity.LOGGER.warn("Item {} not found in set", item);
            }
        }

        if (!src.removeAll(ret)) {
            Scarcity.LOGGER.warn("takeAll array didn't yield anything ({})", Arrays.toString(items));
        }
    ...
    public static <T> Collection<T> takeAll(Set<T> src, Predicate<T> pred) {
        ...
        if (ret.isEmpty()) {
            Scarcity.LOGGER.warn("takeAll predicate yielded nothing", new Throwable());
        }
